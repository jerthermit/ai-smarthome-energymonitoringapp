...
class TelemetryBase(BaseModel):
  ...
  timestamp: datetime
  # Allow zero readings (device off). Was gt=0.
  energy_watts: float = Field(..., ge=0, alias="energyWatts", description="Instantaneous power in watts")
...
class AggregateDataPoint(BaseModel):
  timestamp: datetime
  # Allow zero-energy buckets so windowed APIs can return full calendar sequences without validation errors.
  value: condecimal(ge=0)
